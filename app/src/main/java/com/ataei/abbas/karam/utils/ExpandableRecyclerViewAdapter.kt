 ...
 */
    ...
    private fun onCreateParentView(parent: ViewGroup, viewType: Int): PVH {
        ...
        pvh.containerView.setOnClickListener {
            ...
            else handleExpansion(expandable, position, pvh)
    ...
    private fun collapseAllExcept(position: Int) {
        ...
        if (lastExpandedPosition > 0 && lastExpandedPosition != position) {
...
//            collapseAllExcept(position)
    ...
    private fun handleExpansion(expandableGroup: ExpandableType, position: Int, pvh: PVH) {
        Log.d("bbbb", "expand")
        reverseExpandableState(expandableGroup)
        notifyItemChanged(position)
        onStateChange(pvh, expandableGroup)
    }
     ...
     */
    fun addGroup(expandableGroup: ExpandableType, expanded: Boolean = false, position: Int = 0) {
        ...
        if (position == 0 || position == atPosition)
